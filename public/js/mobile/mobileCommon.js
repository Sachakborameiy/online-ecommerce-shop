function getMobileHomepath() {
	...
	// ios app 웹뷰 화면에서 해당 pathname을 가져올시에 
	// ex) /m2 가 => //m2 '/' 가 하나 더 붙어서 정상적으로 호출이 되지않음  
	try{
		return window.location.pathname.match(/\/([\w-]+)\//)[1] || 'm2';
	}catch(e){
		console.log(e);
	}
}
...
$(function() {
	...
	$(document).on("click", '.faq .article .q', function(e){
		var article = $('.faq .article');
...
function getFaqListData(flag) {
	...
	try {
		$.ajax({
			...
			data: data_param,
			success: function (res) {
				makeFaqList(flag, res.faq_data, itemcd);
			},
			dataType:"json"
		});
	}
...
function makeFaqList(flag, faq_data, itemcd) {
	
	if($("#faq-item-list").length > 0) {

		var item_cnt = $("[name=item_cnt]").val();
		
		var itemnm = $("#faq_cate > option[value='"+itemcd+"']").text();
		...
		if(item_cnt == 0 && faq_data.length <1) {
			...
			if(itemcd =="01"){
				...
				faq_html += "<div class=\"q trigger\"><div class=\"arrow down\"></div><div class=\"title\"><span class=\"sm_title\">["+itemnm+"] </span>" +faq_data[i].question+"</div></div>";
				faq_html += "<div class=\"a\">";
				faq_html += ""+faq_data[i].answer+"</div></div>";
				faq_html += "</li>";
