            ...
            <ul class="list" :data-title="section.title" :data-section="section.section_id">\
                <li v-for="(banners, idx) in section.banners" :data-index="idx + 1">\
                    ...
                    <a v-if="type === \'mobile\'" class="thumb_goods" @click="clickEvent(event, idx)" :style="{ \'background-image\': \'url(\' + banners.image_url + \')\'}">\
                        <img alt="메인배너" src="data:image/png;base64,">\
                    </a>\
                </li>\
            </ul>\
                ...
                <ul class="list" :data-title="section.title" :data-section="section.section_id">\
                    <li v-for="(item, idx) in section.products" :class="{ cut : (idx + 1) % 3 === 0}" :data-index="idx + 1">\
                        <a class="thumb_goods" @click="clickEvent(event, item, idx)">\
                          ...
                          <img class="thumb" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAG0AAACRCAQAAABjof6/AAAANUlEQVR4Ae3BgQAAAADDoPtTT+AI1QAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAIAnfAsAAe7+tYwAAAAASUVORK5CYII=" :style="{ \'background-image\': \'url(\' + item.thumbnail_image_url + \')\'}" alt="상품이미지">\
                        </a>\
                        <div class="info_goods">\
                            ...
                            <span class="price" v-if="parseInt(item.price) >= 0">\
                              <span class="dc" v-if="item.discount_rate > 0">{{ item.discount_rate }}%</span>{{ item.price | commaFilter }}원\
                            </span>\
                            <span class="cost" v-if="item.price !== item.original_price">{{ item.original_price | commaFilter }}원</span>\
                        </div>\
                    </li>\
            ...
            <div class="list_goods">\
                <ul class="list" :data-title="section.title" :data-section="section.section_id">\
                    <li v-for="(item, idx) in section.events" v-if="section.events" :data-index="idx + 1">\
                        <a class="thumb_goods" @click="clickEvent(event, item, idx)">\
                            <img class="thumb" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAF4AAABeCAQAAAAA22vlAAAAAXNSR0IArs4c6QAAACdJREFUeAHtwQENAAAAwiD7p34PBwwAAAAAAAAAAAAAAAAAAAAA4FpFZgABkfKClwAAAABJRU5ErkJggg==" :style="{ \'background-image\': \'url(\' + item.image_url + \')\'}" alt="상품이미지">\
                        </a>\
                        <div class="info_goods">\
            ...
            <ul class="list" :data-title="section.title" :data-section="section.section_id">\
                <li v-for="(item, idx) in section.reviews" :data-index="idx + 1">\
                    <a class="thumb_goods" @click="clickEvent(idx)" target="_blank">\
                        <img class="thumb" :src="item.thumbnail_image_url" alt="상품이미지">\
                    </a>\
                </li>\
            </ul>\
        </div>\
                    ...
                    <li v-for="(item, idx) in section.products" :class="{ cut : (idx + 1) % 3 === 0}" :data-index="idx + 1">\
                        ...
                        <div class="info_goods">\
                            ...
                            <div class="price">\
                              ...
                              <div class="in_price">\
                                <span class="selling" v-if="parseInt(item.price) >= 0">{{ item.price | commaFilter }}원</span>\
                                <span class="cost" v-if="item.price !== item.original_price">{{ item.original_price | commaFilter }}원</span>\
                              </div>\
                            </div>\
                        </div>\
...
var kurlyMain = new Vue({
  ...
  updated: function () {
    this.$nextTick(function () {
      ...
      setTimeout(function () {
        /**
         * @description - shellWrapper를 이용하여 플레이스홀딩 엘리먼트를 데이터가 렌더링 되기전에 fadeOut 시킨 후 삭제
         * @author kurly-jhlim
         */
