...
var lnbMenu = new Vue({
  ...
  methods:{
      ...
      kurlyApi({
        ...
        if(this.type === 'mobile'){
          ...
          if(this.getCategoryNum === '350' || this.getCategoryNum === '351'){
            this.categoryName = '<span class="tit">추석 선물세트</span>';
            this.originalCategoryName = '추석 선물세트';
          }
        }else{
          this.originalCategoryName = this.getData.name.replace(/\//gi,'·');
          this.categoryName = this.originalCategoryName;
          if(this.getCategoryNum === '350' || this.getCategoryNum === '351'){
            this.categoryName = '추석 선물세트';
            this.originalCategoryName = '추석 선물세트';
          }
        }
        ...
        if(this.type === 'mobile' && (this.getCategoryNum == '029' || this.getCategoryNum == '038' || this.getCategoryNum == '015' || (this.getCategoryNum == '919' && this.mainType === true) ) ){
          ...
          /**
           * show_all_flag 는 상품 목록에서 전체보기 사용 여부를 판명함.
           * 전체보기 | 하위메뉴1 | 하위메뉴2
           * - 전체보기 하나만 있어야 하는경우 => show_all_flag: true
           * - 전체보기 외에 다른 메뉴가 있는경우 => show_all_flag: boolean
           */
    ...
    ,trackerAction: function(item){ // Amplitude 연동 (2020-07, KM-2919) : Update
      var that = this;
    ...
    ,trackerDataUpdate: function(){ // Amplitude 연동 (2020-07, KM-2919) : Update
      var that = this;
      if(typeof data === 'undefined' && typeof that.trackingActionData.primary_category_id === 'undefined'){
        that.trackingActionData = {
          ...
          secondary_category_name: that.selectCategoryName === null ? '전체보기' : that.selectCategoryName
        }
      }
...
Vue.component('category-banner',{
  ...
  template:'\
    <div>\
        <div class="bnr_category" v-if="categoryBannerLink != \'\'"><a :href="categoryBannerLink" @click="clickAction(event)"><img :src="categoryBannerUrl" alt="카테고리 배너"></a></div>\
        <div class="bnr_category" v-if="categoryBannerLink == \'\'"><img :src="categoryBannerUrl" @click="clickAction(event)" alt="카테고리 배너"></div>\
    </div>\
    '
  ,methods: {
    clickAction: function(e){ // Amplitude 연동 (2020-07, KM-2919)
      e.preventDefault();
      var that = this;
...
Vue.component('list-goods',{
        ...
        <div class="item">\
            <div class="thumb">\
                ...
                <div class="group_btn">\
                    ...
                    <button class="btn btn_alarm on" @click="notifyAction(item, idx+pageCount)" type="button" v-if="item.use_stocked_notify && (item.package_soldout || item.sold_out)"><span class="screen_out">재고알림 신청</span></button>\
            ...
            <a @click="clickAction(event, item, idx+pageCount)" class="info">\
                ...
                <span class="cost">\
                    ...
                    <span class="original" v-if="item.original_price != item.price">{{ item.original_price | commaFilter }}원</span>\
                </span>\
  ...
  ,methods:{
    ...
    , notifyAction:function(item, count){ // 재입고 알림
      // KM-1483 Amplitude
...
var goodsList = new Vue({
  ...
  data: {
    ...
    tracking: { // Amplitude 용 데이터는 여기에 담기
      ...
      search: {
        packageCount: 0, // 검색된 상품 수 (KM-4988)
        keyword: '', // 상품 검색 키워드 (KM-4988)
      },
    },
    isMainType: false, // lnb가 main부분인지 아닌지를 구분하는 값
    loginCheck : false,
    searchResult: false, // 검색실행할시 해당값 true로 변경됨.
    type: 'mobile',
  },
  mounted: function () {
  ...
  methods:{
    ...
    , trackingCategoryBanner: function(){ // Amplitude 연동 (2020-07, KM-2919)
      ...
      // KMF-61 베너 링크가 활성화 되었을때만 select_category_banner 이벤트 발생
  ...
  updated : function(){
    this.$nextTick(function (){
      ...
      if($self.type === 'mobile'){
        ...
        var $goodsList = $('#goodsList');  // !$goodsList.hasClass('off') : 검색결과에서만 사용됨.
